...
bool preguntarSiNo() {
  ...
  while (true) {
    if (digitalRead(PIN_BUTTON_SR) == LOW) {
      delay(200);
      while (digitalRead(PIN_BUTTON_SR) == LOW) delay(10);
      delay(300); // Delay adicional para evitar rebotes
      return true;
    }
    if (digitalRead(PIN_BUTTON_SL) == LOW) {
      delay(200);
      while (digitalRead(PIN_BUTTON_SL) == LOW) delay(10);
      delay(300); // Delay adicional para evitar rebotes
      return false;
    }
    delay(10);
